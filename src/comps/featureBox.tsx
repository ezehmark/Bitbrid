...
import styles from "./featureBox.module.css"; // Correct relative path for CSS Module

export default function FeatureBox({ day }) {
  // First circle animation properties
  ...
  // Second part of the first circle (Circle1b) animation properties
  ...
  // Inner circle (Circle 2) animation properties
  ...
  // Second part of the inner circle (Circle 2b) animation properties
  ...
  // State for displaying the current feature text
  ...
  useEffect(() => {
    ...
    const runCycle = () => {
      ...
      // Use styles.featureAnimClass and styles.featureAnimNight
      c1Ref.current?.classList.remove(day ? styles.featureAnimClass : styles.featureAnimNight);
      c1bRef.current?.classList.remove(day ? styles.featureAnimClass : styles.featureAnimNight);
      c2Ref.current?.classList.remove(day ? styles.featureAnimClass : styles.featureAnimNight);
      c2bRef.current?.classList.remove(day ? styles.featureAnimClass : styles.featureAnimNight);

      setTimeout(() => {
        c1Ref.current?.classList.add(day ? styles.featureAnimClass : styles.featureAnimNight);
      }, 5000);
      setTimeout(() => {
        c1bRef.current?.classList.add(day ? styles.featureAnimClass : styles.featureAnimNight);
      }, 11000);
      setTimeout(() => {
        c2Ref.current?.classList.add(day ? styles.featureAnimClass : styles.featureAnimNight);
      }, 16000);
      setTimeout(() => {
        c2bRef.current?.classList.add(day ? styles.featureAnimClass : styles.featureAnimNight);
      }, 21000);
    };
    ...
    runCycle();
    cycleInterval = setInterval(runCycle, 24000);
  ...
  return (
    <div
      className={styles.featureBox} // Apply class from CSS Module
      style={{ backgroundColor: "transparent", width: windowWidth }}
    >
      <div
        className={styles.circle1} // Apply class from CSS Module
        style={{
          position: "relative",
          height: 260,
          width: 260,
      ...
      >
        ...
        >
          <img
            ...
            alt="Efficient system icon"
            style={{ position: "absolute", height: "125%", width: "125%" }}
          />
        </motion.div>

        <div
          className={styles.circle2} // Apply class from CSS Module
          style={{
            position: "absolute",
            height: 160,
            width: 160,
        ...
        >
          ...
          {feature && (
            <div ref={featureRef} className={styles.feature}> {/* Apply class from CSS Module */}
              {feature}
            </div>
          )}
